...
namespace Quokka.Stomp
{
	public class StompFrameBuilder : IFrameBuilder<StompFrame>
	{
		...
		public ArraySegment<byte> ToArray(StompFrame frame)
		{
			if (frame == null)
			{
				frame = StompFrame.HeartBeat;
			}
			...
			if (data.Length > 3
				...
				&& data[2] == 0xbf)
			{
				// do not include the UTF-8 byte order mark 
				return new ArraySegment<byte>(data, 3, data.Length - 3);
			}
		...
		private void ReadCommandLine(byte[] data, int offset, int length)
		{
			...
			// Check for a newline
			if (ByteArrayUtil.SkipNewLine(data, ref offset, ref length))
			{
				int count = 1;
				// Skip any more leading newlines
				...
				if (Log.IsDebugEnabled)
				{
					...
					{
						Log.Debug("RX: " + frame + " * " + count);
					}
				}

				_readyFrames.Enqueue(frame);
			}
			...
			_frameUnderConstruction = new StompFrame(commandText);
		...
		private void AddBodyToFrameAndAddFrameToReadyQueue(byte[] data, int offset, int length)
		{
			...
			if (Log.IsDebugEnabled)
			{
				Log.Debug("RX: " + _frameUnderConstruction);
			}
